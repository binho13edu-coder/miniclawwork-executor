...
const askLLM = async (t) => {
    ...
    const call = async (u, k, m) => {
        const res = await axios.post(u, { 
            model: m, messages: msgs, temperature: 0.1, max_tokens: 150 
        }, { headers: { 'Authorization': `Bearer ${k}` }, timeout: 10000 });
        return res.data.choices[0].message.content;
    };
    
    try { 
        const ans = await call('https://openrouter.ai/api/v1/chat/completions', 
            process.env.OPENROUTER_API_KEY, 'meta-llama/llama-3.1-8b-instruct:free');
        conversationHistory.push({role:'assistant', content: ans});
        return ans;
    } catch (e1) {
        console.log('[OR fail]', e1.message);
        try { 
            const ans = await call('https://api.groq.com/openai/v1/chat/completions', 
                process.env.GROQ_API_KEY, 'llama-3.1-8b-instant');
            conversationHistory.push({role:'assistant', content: ans});
            return ans;
        } catch (e2) {
            console.log('[Groq fail]', e2.message);
            try { 
                const ans = await call('https://integrate.api.nvidia.com/v1/chat/completions', 
                    process.env.NVIDIA_API_KEY, 'meta/llama-3.1-8b-instruct');
                conversationHistory.push({role:'assistant', content: ans});
                return ans;
            } catch (e3) { 
                console.log('[NVIDIA fail]', e3.message);
                return "⚠️ Offline."; 
            }
        }
    }
